...
def plot_1D(xdata, ydata, zdata, xname, yname, zname):
    ...
    if np.shape(xdata) == np.shape(zdata):
        fig_1D.add_trace(go.Scatter(
            x=xdata, y=zdata, name=f'{yname} = {ydata}', mode='lines+markers'
        ))
    else:
        for i, data in enumerate(zdata):
            fig_1D.add_trace(go.Scatter(
                x=xdata, y=data, name=f'{yname} = {ydata[i]}', mode='lines+markers'
            ))
...
def plot(xdata, ydata, zdata, xname, yname, zname):
    ...
    if np.shape(xdata) == np.shape(zdata):
        fig.add_trace(go.Scatter(visible=False, x=xdata, y=zdata, name=f'{yname} = {ydata:.2f}'))
    else:
        for i, data in enumerate(zdata):
            fig.add_trace(go.Scatter(visible=False, x=xdata, y=data, name=f'{yname} = {ydata[i]:.2f}'))
